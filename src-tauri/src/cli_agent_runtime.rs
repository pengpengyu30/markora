    ...
    H: FnMut(&serde_json::Value, &mut F, &mut String),
{
    ...
    let mut child = spawn_json_line_child(&mut process)?;
...
fn spawn_json_line_child(process: &mut JsonLineProcess<'_>) -> Result<std::process::Child, String> {
    process
        .command
        .spawn()
...
#[cfg(test)]
#[path = "cli_agent_runtime/tests.rs"]
mod tests;
