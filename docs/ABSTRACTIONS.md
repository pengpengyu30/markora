...
Standalone `.html` and `.htm` vault entries load as ordinary text tabs, but default to the generic `FilePreview` fallback instead of an in-app renderer. The existing raw-source preference is shared with Markdown: the breadcrumb source action or `Cmd/Ctrl+\` swaps the fallback for CodeMirror, and the fallback's external-open command remains the escape hatch for browser-only behavior. The breadcrumb derives action availability from the entry kind: standalone HTML keeps source, file location, link, and delete actions while omitting Markdown metadata, neighborhood, reading-width, table-of-contents, archive, and PDF-export controls.

Rendered note PDF export is not a stored file-kind transformation. `src/utils/notePdfExport.ts` temporarily marks the current webview for print-only rendering, asks for a `.pdf` filesystem destination only when the native capability reports direct save support, and invokes Tauri's native `WKWebView` PDF export command on macOS. Windows/Linux Tauri builds and browser mode keep print-dialog fallback behavior. `src/components/useEditorPdfExport.ts` ensures the rendered Markdown view is active before export. Markdown frontmatter is ignored and the source file is never modified.

### Legacy Sheet Notes

Notes with `_display: sheet` are legacy Markdown files. The current editor no longer evaluates their CSV body or opens a dedicated spreadsheet surface; it keeps the source text available through the generic unsupported-file fallback and raw mode. The compatibility reader still recognizes the marker so older notes do not get silently routed through the normal rich-editor parser. New notes are always ordinary Markdown, and no new `_sheet` metadata is written.
...
- The rich editor renders the block with the `tldraw` package and hydrates a persisted snapshot into the store before the board mounts. Debounced snapshot changes are flushed before external editor actions (including note switches and saves), with unmount cleanup as a final safety net, so BlockNote serialization cannot overwrite the latest drawing with stale props.
...
- The tldraw runtime receives Tolaria's resolved light/dark mode and an explicit mapped locale as its user preferences, so embedded whiteboards follow the app appearance and language instead of the browser/system default.
...
Fenced `html` blocks are ordinary Markdown code blocks. They use the normal code-block path, never execute expressions or scripts, and never mount an iframe. Standalone `.html`/`.htm` files use the generic `FilePreview` fallback; raw CodeMirror remains available for direct source editing. New HTML blocks cannot be inserted from the slash menu.
...
- `focusOwnershipGuard.ts` is the shared global focus interception primitive for editor-like surfaces. It owns the single `HTMLElement.prototype.focus` patch, document focus/pointer listeners, outside-target restoration, and cleanup; rich-editor modules keep only their surface-specific focus-claim policy.
