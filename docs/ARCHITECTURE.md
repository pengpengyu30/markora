...
| Raw editor | CodeMirror 6 + official language packages | Markdown, standalone HTML, YAML, JSON, Python, SQL, JS/TS |
...
Tags are note-level YAML frontmatter stored under the standard `tags` key as a scalar or
array. The renderer normalizes both forms to a trimmed, lowercased, deduplicated string
array, while the line-oriented frontmatter writer preserves every unrelated frontmatter
and body byte. The editor breadcrumb exposes existing tags as removable chips below the
filename and places an icon-only Add tag picker in the right toolbar. New tags are
lowercased and must match `[a-z0-9]+(?:-[a-z0-9]+)*` with a maximum length of 15
characters; unsupported symbols, underscores, repeated separators, and overlong values
are rejected. Body text is never parsed for tags.

The sidebar derives usage counts from the unfiltered visible entry set and renders a
collapsible Tags section above the Projects/folder tree; it is collapsed by default while
remembering an explicit user expansion. Tag selections are installation-session state,
independent from folder selection, and filter the note list with AND semantics.
The note-list header shows the active tag chips and a Clear action. Reloading or restarting
the app clears the selection; the `tags` frontmatter remains the durable source of truth.
...
- **Editor** (flex, fills remaining space): Single note open at a time (no tabs — see ADR-0003). Breadcrumb bar with filename controls, read-only legacy display-title context when a no-H1 note's title differs from its filename, word count, rich-editor width toggle, and the secondary-overflow Table of Contents action, BlockNote rich text editor with wikilink support, Markdown-compatible inline/display math rendering, editable Obsidian/GFM-style blockquote callouts, first-class Mermaid diagram blocks, markdown-safe formatting controls, and schema-backed fenced code block highlighting via `@blocknote/code-block` plus lazy direct `@shikijs/langs` registrations for missing common grammars. Callout bodies retain rich inline marks and links, known aliases use semantic app colors, unknown types use neutral styling, and `+`/`-` fold markers control only the initial disclosure state while remaining unchanged in Markdown. Fenced `html` content remains an ordinary Markdown code block: it never executes, evaluates vault expressions, mounts an iframe, or takes over the note surface. Legacy sheet notes stay as source text and show the generic `FilePreview` fallback. Can toggle to diff view (modified files), raw CodeMirror view, or a wide rich-editor reading surface with preserved side margins; raw CodeMirror remains full-width and unaffected by note width mode. Standalone HTML/HTM files use the generic unsupported-file fallback; the existing breadcrumb source action or `Cmd/Ctrl+\` opens CodeMirror for direct editing, and the fallback's external-open action is the escape hatch for browser-only behavior. Their breadcrumb keeps file/source actions while suppressing Markdown-only favorite, organized, neighborhood, width, table-of-contents, archive, and PDF-export controls. Raw CodeMirror chooses syntax highlighting from the file extension for Markdown, YAML, JSON, HTML, Python, SQL, JavaScript, and TypeScript files, and keeps unknown text files plain. Inline rich-editor images open in a localized shadcn lightbox on double-click while normal single-click BlockNote selection remains untouched, and tiny tracking-style images are ignored. Binary image, audio, video, and PDF files render through `FilePreview` as ordinary vault files using Tauri asset URLs; editor-embedded audio and video use the same scoped asset sources through the CSP `media-src` allow-list, while packaged PDF previews require scoped asset sources in both `object-src` and `frame-src` because the webview PDF renderer uses a nested frame context. Linux AppImage builds ask the native runtime whether audio/video should fall back to external-open controls before mounting webview media elements. External-open actions call `open_vault_file_external` so the target is validated against the active vault before the native default app opens it. Unsupported/broken binaries show explicit fallback states and keyboard focus returns to the note list on `Escape`. Decomposed into `Editor` (orchestrator), `EditorContent`, `FilePreview`, `EditorRightPanel`, `TableOfContentsPanel`, `SingleEditorView`, with hooks `useDiffMode`, `useEditorFocus`, and `useEditorSave`, plus the `useRawMode`/`RawEditorView` pair for raw source editing. Rich BlockNote input and raw CodeMirror input both route typed `->`, `<-`, and `<->` through the shared `src/utils/arrowLigatures.ts` resolver so arrow ligatures stay consistent across mode switches while escaped ASCII sequences remain literal. Rich-editor Markdown input transforms for arrows, inline math, and `==highlight==` share one capture-phase `beforeinput` execution path in `src/components/richEditorInputTransform.ts` and are composed by `src/components/richEditorInputTransformExtension.ts`. Navigation history (Cmd+[/]) replaces tabs.
  ...
  Note PDF export stays renderer-owned for layout: `useEditorPdfExport` exits diff/raw views, applies a print-only stylesheet to the rendered Markdown root, and checks the native PDF capability before choosing a platform path. On macOS, the renderer asks for a filesystem PDF destination before the Tauri `export_current_webview_pdf` command saves the current `WKWebView` print operation directly; on Windows/Linux Tauri builds and in browser mode, the same export action falls back to the native/browser print dialog. Markdown export reuses rendered BlockNote output so frontmatter is omitted; source files are never modified. Markdown notes expose the export action from Cmd+K, the native Note menu, the breadcrumb overflow menu, and each Markdown row's note-list context menu.
...
| `git/` | Git operations and shared system-git helpers (`command.rs`, `remote_config.rs`, `commit.rs`, `status.rs`, `history.rs`, `recovery.rs`, `conflict.rs`, `remote.rs`, `pulse.rs`, `clone.rs`, `connect.rs`) |
...
| `list_deleted_notes` | List Markdown notes currently missing from the local snapshot |
| `get_deleted_note_preview` | Read the pre-deletion content of a note pending restoration |
| `restore_deleted_note` | Write the pre-deletion content back to the vault and create a new Tolaria safety snapshot |
