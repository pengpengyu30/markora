...
describe('linked code Markdown', () => {
  ...
  it('leaves malformed linked-code tokens untouched', () => {
    const malformedToken = '\u2039LINKED_CODE:1--2\u203A'
    const block = {
      type: 'paragraph',
      content: [{
        type: 'link',
        href: 'https://example.com',
        content: [{ type: 'text', text: malformedToken, styles: {} }],
      }],
    }

    const [result] = injectLinkedCodeInBlocks([block])
