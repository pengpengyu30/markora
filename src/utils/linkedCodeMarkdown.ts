...
function decodeCode(value: string): string | null {
  const codePoints = value.split('-').map(part => Number.parseInt(part, 16))
...
function linkedCodePart(item: InlineItem, part: string, index: number): InlineItem[] | null {
  if (!part) return []
  if (index % 2 === 0) return [{ ...item, text: part }]
  const decoded = decodeCode(part)
  if (decoded === null) return null
  return [{ ...item, text: decoded, styles: { ...item.styles, code: true } }]
}
...
function expandLinkedCodeText(item: InlineItem): InlineItem[] {
  ...
  const expanded: InlineItem[] = []
  for (const [index, part] of parts.entries()) {
    const nextItems = linkedCodePart(item, part, index)
    if (nextItems === null) return [item]
    expanded.push(...nextItems)
  }
  return expanded
}
