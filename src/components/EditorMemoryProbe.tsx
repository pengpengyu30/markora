...
declare global {
  interface Window {
    __markoraEditorMemoryProbe?: EditorMemoryProbeApi
  }
}
...
function useEditorMemoryProbeBridge(api: EditorMemoryProbeApi): void {
  useEffect(() => {
    ...
    window.__markoraEditorMemoryProbe = api
    return () => {
      if (window.__markoraEditorMemoryProbe?.run === api.run) {
        delete window.__markoraEditorMemoryProbe
      }
    }
  }, [api])
}
