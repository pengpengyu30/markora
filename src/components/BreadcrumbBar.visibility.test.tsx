...
describe('BreadcrumbBar filename visibility', () => {
  ...
  it('offsets the editor-only breadcrumb title past the macOS traffic lights', () => {
    ...
    expect(editorCss).toContain('--breadcrumb-bar-left-padding: var(--markora-macos-traffic-light-padding, 90px);')
  })
  ...
  it('does not keep removed sheet-editor layout rules', () => {
    const editorCss = readFileSync(`${process.cwd()}/src/components/Editor.css`, 'utf8')

    expect(editorCss).not.toContain('.editor-scroll-area--sheet')
  })
})
