...
async function readSlashMenuItemStyles(item: Locator) {
  return item.evaluate((node) => {
    ...
    const regularIcon = item.querySelector<HTMLElement>('.markora-slash-menu-icon__regular')!
    const fillIcon = item.querySelector<HTMLElement>('.markora-slash-menu-icon__fill')!
...
test.describe('BlockNote slash menu styling', () => {
  ...
  test('slash menu keeps Mantine styling tokens in the editor', async ({ page }) => {
    ...
    await expect(secondItem.locator('.markora-slash-menu-icon')).toBeVisible()
